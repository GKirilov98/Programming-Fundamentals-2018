...
namespace Problem_3___Anonymous_Vox
{
    class AnovymousVox
    {
        ...
        { //Comments bellow
            ...
            foreach (Match match in matches)
            {
                ...
                input = Replace(input, match.Groups[2].Value, values[index]);
                index++;
            }
        ...
        {
             string substringOld = text.Substring(0, text.IndexOf(oldValue) + oldValue.Length);
             string replace = substringOld.Replace(oldValue, newValue);
            string fuseNewText = replace + text.Substring(substringOld.Length);
